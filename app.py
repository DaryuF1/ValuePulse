...
def find_top_tickets(matches, target_odds, tolerance, top_n, max_overlap, min_matches):
    valid_tickets = []

    def backtrack(index, current_ticket, current_odds, current_prob, current_matches_set):
        if current_odds > target_odds + tolerance:
            return
        if abs(current_odds - target_odds) <= tolerance and len(current_ticket) >= min_matches:
...
def salveaza_bilet_istoric(ticket_data):
    istoric = []
    if os.path.exists('bilete_salvate.json'):
        try:
            with open('bilete_salvate.json', 'r', encoding='utf-8') as f:
                istoric = json.load(f)
        except:
            istoric = []
    ...
    with open('bilete_salvate.json', 'w', encoding='utf-8') as f:
        json.dump(istoric, f, indent=4)


def verifica_status_bilete():
    if not os.path.exists('bilete_salvate.json'):
        return []
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    for f in fixtures:
                        ...
                        if home_t.lower() in h_name.lower() or away_t.lower() in a_name.lower():
                            match_found = True
                            hg = f['goals']['home']
                            ag = f['goals']['away']
...
with st.sidebar:
    ...
    if mode == "Selectează manual":
        selected_names = st.multiselect("Alege ligile active", list(SPORTS.keys()), default=["Premier League"])
        active_leagues = [SPORTS[name] for name in selected_names]

    profile = st.selectbox("Profil Bilet", ["SAFE (2.0)", "MEDIUM (5.0)", "RISKY (15.0)", "Personalizat"])

    if profile == "Personalizat":
        target_odds = st.number_input("Cota țintă", min_value=1.1, max_value=100.0, value=5.0)
        tolerance = st.number_input("Toleranță cotă (+/-)", min_value=0.05, max_value=5.0, value=0.5)
    else:
        target_odds, tolerance = {"SAFE (2.0)": (2.0, 0.2), "MEDIUM (5.0)": (5.0, 0.5), "RISKY (15.0)": (15.0, 2.0)}[
            profile]

    bet_style = st.radio("Stil Bilet", ["Scurt (2-3 meciuri)", "Lung (4+ meciuri)"])
...
if st.session_state.top_tickets:
    ...
    for i, t in enumerate(st.session_state.top_tickets, 1):
        st.subheader(f"Varianta {i} — Cota: {t['odds']:.2f} (Prob: {t['prob'] * 100:.1f}%)")
        ...
        with col1:
            if st.button(f"💾 Salvează în Istoric V{i}", key=f"save_hist_{i}"):
                ...
                st.success(f"Varianta {i} a fost salvată în istoric pentru urmărire!")
    ...
    if generate_btn:
        st.warning(
            "Nu s-au găsit meciuri care să respecte criteriile stricte pentru cota aleasă. Încearcă o toleranță mai mare sau alte ligi.")
    ...
    try:
        ...
        if saved_tickets:
            for sb in saved_tickets:
                status_color = "🟡" if sb['status'] == "În desfășurare" else ("🟢" if "Câștigat" in sb['status'] else "🔴")
                with st.expander(
                        f"Bilet #{sb['id']} | Dată: {sb['data']} | Cota: {sb['cota']:.2f} | Status: {status_color} {sb['status']}"):
                    for b in sb['ticket']:
                        st.write(f"- {b['match']} | **{b['selection']}** (Cota: {b['odds']:.2f})")
        else:
            st.info("Nu există bilete salvate în istoric momentan.")
    except:
        st.info("Nu există bilete salvate în istoric momentan.")
else:
    st.info("Nu există fișierul de istoric creat încă. Salvează un bilet ca să începi urmărirea.")